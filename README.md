...
| `/배고파` | 점심 전에는 점심까지, 점심 종료 후에는 퇴근까지 남은 시간을 표시합니다. |
| `/점심시간설정 시간:12:50` | 서버 공통 점심시간을 `HH:MM` 형식으로 설정합니다. 서버 관리 권한이 필요합니다. |
| `/퇴근시간설정 시간:17:50` | 서버 공통 퇴근시간을 `HH:MM` 형식으로 설정합니다. 서버 관리 권한이 필요합니다. |

봇은 한국 표준시(KST)를 사용합니다. 식단표 게시일에 7일을 더해 실제 메뉴 주차를 계산하므로,
공휴일 등의 이유로 게시물이 금요일보다 일찍 올라와도 다음 주 식단표로 처리합니다.
`/배고파`는 주말에는 휴무를 안내하고, 평일에는 점심 시작 전까지 점심 카운트다운을 표시합니다.
점심 시작 후 70분 동안은 식사 시간을 안내하며, 이후에는 퇴근 카운트다운 또는 퇴근 후 귀가 안내를 표시합니다.
...
mega_lunch/
  ...
  schedule.py             서버별 점심·퇴근 일정 검증, 계산 및 저장
...
tests/
  ...
  test_schedule.py        서버 일정과 시간대별 배고파 응답 판정 테스트
...
- 점심·퇴근시간 입력 검증, 시간대별 `/배고파` 판정과 서버별 설정 저장
...
- 서버별 점심·퇴근시간 설정은 `cache/guild-schedules.json`에 저장합니다. 점심시간은 기본 12:00이며 70분 동안 적용됩니다.

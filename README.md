...
mega_lunch/
  ...
  prefetch.py             다음 주 식단표 사전 캐시 일정 계산
  service.py              게시물 조회와 캐시 조정
  settings.py             환경변수와 실행 설정
  models.py               내부 데이터 모델
tests/
  test_cache.py           식단표 경계 검출과 크롭 폴백 테스트
  test_calendar.py        주차 경계와 내일 메뉴 계산 테스트
  test_prefetch.py        사전 캐시 요일, 대상 주차와 실행 시각 테스트
```
...
- 식단표 위치 변화에 따른 점심 영역 경계 검출과 설정값 폴백
- 토요일 사전 캐시 대상 주차와 KST 오전 9시 실행 설정
...
- 봇이 실행 중이면 매주 토요일 오전 9시(KST)에 다음 주 식단표를 미리 캐시해 최초 명령의 응답 지연을 줄입니다.
- 요일별 이미지는 식단표의 가로 경계를 감지해 날짜 행부터 Take-Out 직전까지 자릅니다. 경계를 찾지 못하면 크롭 설정값을 사용합니다.
